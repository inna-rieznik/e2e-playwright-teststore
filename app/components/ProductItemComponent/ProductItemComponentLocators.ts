...
export default class ProductItemComponentLocators extends BaseLocators {
  ...
  readonly discountFlag: Locator = this.baseLocator.locator(
    'li[class*="discount"]'
  );
  readonly priceRow: Locator = this.baseLocator.locator(
    '[class="product-price-and-shipping"]'
  );
  readonly productTitleRow: Locator = this.baseLocator.locator(
    '[class*="product-title"]'
  );
